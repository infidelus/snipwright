#!/usr/bin/env bash
#
# Install Snipwright application-menu entries (editor + watcher) for the current
...
cat > "$APPS/snipwright.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=Snipwright
...
# offers "Open with Snipwright" on them and shows the app icon.  Without this the
# desktop has no idea what a .vprj is.
MIME="$HOME/.local/share/mime"
mkdir -p "$MIME/packages"
cat > "$MIME/packages/snipwright.xml" <<EOF
...
# desktop entry's MimeType line is enough to make Snipwright appear in the file
# manager's "Open With" list for .vprj files.  We deliberately do NOT call
# `xdg-mime default` here: that would make Snipwright the sole handler and remove
...
Icon=$ICON
Terminal=false
StartupWMClass=snipwright-watcher
...
echo "  $APPS/snipwright.desktop"
echo "  $APPS/snipwright-watcher.desktop"
echo "Python: $PY"
echo "Icon:   $ICON"
echo
